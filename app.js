...
return {
url: ‘data:image/svg+xml;charset=UTF-8,’ + encodeURIComponent(` <svg xmlns="http://www.w3.org/2000/svg" width="40" height="52" viewBox="0 0 40 52"> <path d="M20 0C9 0 0 9 0 20c0 15 20 32 20 32S40 35 40 20C40 9 31 0 20 0z" fill="#FFD700" stroke="#C5A059" stroke-width="2"/> <circle cx="20" cy="19" r="11" fill="#1A1D23"/> <text x="20" y="24" text-anchor="middle" font-size="14" fill="#FFD700" font-family="Arial">★</text> </svg>`),
...
```
// หน่วงนิดนึงให้ modal render ก่อน
setTimeout(() => {
    try {
        ...
        _qrScanner.start(
            ...
            (decodedText) => {
                // สแกนได้แล้ว หยุดกล้อง
                _qrScanner.stop().catch(() => {});
                document.getElementById('qrScanInput').value = decodedText;
                verifyAndUseDeal();
            },
            () => {} // error เงียบ ให้สแกนต่อ
        ).catch(err => {
            document.getElementById('qrReaderBox').innerHTML =
                `<p style="color:#D9534F;text-align:center;padding:20px;font-size:13px;">
                    ❌ ไม่สามารถเปิดกล้องได้<br>
                    <span style="color:#888;font-size:11px;">${err}</span>
                </p>`;
        });
    } catch(e) {
        document.getElementById('qrReaderBox').innerHTML =
            `<p style="color:#D9534F;text-align:center;padding:20px;">❌ Html5Qrcode ไม่พร้อมใช้งาน</p>`;
    }
}, 300);
```
...
: available.map(r => ` <div style="background:rgba(0,0,0,0.4); border:1px solid var(--info); padding:15px; border-radius:12px; margin-bottom:12px; text-align:left; box-shadow:0 4px 10px rgba(0,0,0,0.3);"> <h4 style="margin:0 0 5px; color:#FFF; font-size:16px;">🎁 ${r.name}</h4> <p style="margin:0 0 12px; font-size:12px; color:#888;">ได้รับเมื่อ: ${r.date}</p> <button class="btn-primary" style="background:linear-gradient(135deg,#17a2b8,#00d2ff); color:#FFF; width:100%; border:none; padding:10px; font-size:14px; border-radius:8px;" onclick="claimReward('${r.id}')"> กดใช้สิทธิ์ (แสดงหน้าจอให้ร้านดู) </button> </div>`).join(’’);
...
return ` <div style="display:flex;align-items:center;justify-content:space-between;gap:5px; background:rgba(0,0,0,0.3);padding:10px;border-radius:8px; border:1px solid rgba(255,255,255,0.1);margin-bottom:5px;"> <span style="width:60px;font-size:14px;color:#FFF;font-weight:bold;">${daysTH[i]}</span> <div style="display:flex;align-items:center;gap:5px;"> <input type="time" id="open_${i}"  value="${d.open}"  style="padding:8px;font-size:14px;width:100px;border-radius:5px;" ${d.isClosed ? 'disabled' : ''}> <span style="color:#999;">-</span> <input type="time" id="close_${i}" value="${d.close}" style="padding:8px;font-size:14px;width:100px;border-radius:5px;" ${d.isClosed ? 'disabled' : ''}> </div> <label style="font-size:13px;display:flex;align-items:center;gap:5px;color:var(--danger);font-weight:bold;margin-left:5px;"> <input type="checkbox" id="closed_${i}" ${d.isClosed ? 'checked' : ''} onchange="toggleDayStatus(${i})" style="width:18px;height:18px;"> ปิด </label> </div>`;
}).join(’’);
}
